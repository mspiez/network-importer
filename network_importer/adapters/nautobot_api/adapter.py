...
class NautobotAPIAdapter(BaseAdapter):
    ...
    def load(self):
        ...
        for location in self.get_all(self.location):
            self.load_nautobot_prefix(location)
            self.load_nautobot_vlan(location)
        ...
        # Load Cabling
        for location in self.get_all(self.location):
            self.load_nautobot_cable(location=location, device_names=device_names)

    def load_nautobot_device(self, location, device):
        ...
        self.load_nautobot_ip_address(location=location, device=device)

    def load_nautobot_prefix(self, location):
    ...
    def load_nautobot_interface(self, location, device):
        ...
        for intf in intfs:
            self.convert_interface_from_nautobot(location=location, device=device, intf=intf)

        LOGGER.debug("%s | Found %s interfaces for %s", self.name, len(intfs), device.name)

    def load_nautobot_ip_address(self, location, device):  # pylint: disable=unused-argument
    ...
    def load_nautobot_cable(self, location, device_names):
        """Import all Cables from Nautobot for a given location.
